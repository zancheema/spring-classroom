...
class ClassroomStudentOrTeacherFilterTest {
    @Mock
    ...
    private ClassroomStudentOrTeacherFilter filter;

    @BeforeEach
    public void setup() {
        request = new MockHttpServletRequest();
        response = new MockHttpServletResponse();
        filter = new ClassroomStudentOrTeacherFilter(securityService, securityContext);
    }
    ...
    @Test
    public void userIsStudentShouldPassThroughTheFilter() throws ServletException, IOException {
        when(securityContext.getAuthentication())
                .thenReturn(authentication);
        User user = new User();
        user.setId(1);
